...
```bash
# Test date conversion on a single date
python -m fec dates test "12/31/2024"

# Preview migration (dry run)
python -m fec dates migrate --dry-run

# Migrate data files with date columns
python -m fec dates migrate
```

Note: Future data updates via `python -m fec update run` automatically apply date conversion during processing.
